...
@section('content')
    <main class="h-full overflow-y-auto">
        <button type="button"
            ...
            <a href="/admin/activities/create" class="">Tambah Data</a>
        </button>
                ...
                <table class="w-full table-auto border-separate whitespace-no-wrap">
                    <thead>
                        <tr
                            ...
                            <th class="px-4 py-3 w-20">Tanggal Kegiatan</th>
                    ...
                    <tbody class="bg-white divide-y dark:divide-gray-700 dark:bg-gray-800">
                        ...
                        @foreach ($activities as $activity)
                            <tr
                                ...
                                <td class="px-4 py-3 text-center">{{ $activity->date }}</td>
                                <td class="px-4 py-3">{{ $activity->title }}</td>
                                <td class="px-4 py-3">{{ $activity->description }}</td>
                                <td class="px-4 py-3">
                                    <img src="{{ asset('image/' . $activity->image) }}" alt=""
