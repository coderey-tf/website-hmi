        ...
        <form action="{{ route('activities.update', $activity->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
                <div class="block text-lg form-group">
                    <label for="" class="text-gray-700 dark:text-gray-400">Tanggal Kegiatan</label>
                    <input
                ...
                <div class="block mt-4 text-lg form-group">
                    ...
                    <input
                        class="block w-full mt-1 text-lg dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input"
                        placeholder="Judul" name="title" value="{{ $activity->title }}" />
                </div>
                @error('title')
                    <small style="color:red">{{ $message }}</small>
                @enderror
                ...
                <div class="block mt-4 text-lg form-group">
                    ...
                    <textarea
                        ...
                        rows="3" placeholder="Deskripsi" name="description">{{ $activity->description }}</textarea>
                </div>
                @error('description')
                    <small style="color:red">{{ $message }}</small>
                @enderror

                <img src="{{ asset('image/' . $activity->image) }}" alt="" class="img-fluid mt-4" width="150">
